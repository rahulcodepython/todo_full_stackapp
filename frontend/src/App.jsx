...
export default function App() {
        ...
        fetch('http://127.0.0.1:8000/api/token/', options)
            ...
            .then(response => {
                ...
                if (response.detail) {
                    ...
                    console.log("Lets set the cookies of token and refresh token");
                    setCookie('token', response.access, { 'maxAge': `${60 * 5}` });
                    setCookie('refreshToken', response.refresh, { 'maxAge': `${60 * 5}` });
                    console.log("previousLogin cookie is now set true");
                    setCookie('previousLogin', JSON.stringify(true), { 'maxAge': `${60 * 5}` });
    ...
    // const pasreTheTokenGetFromRefreshToken = async (response) => {
    //     console.log("pasreTheTokenGetFromRefreshToken starts");
    ...
    //     console.log("pasreTheTokenGetFromRefreshToken ends");
    // }
    ...
    const getTokenFromRefreshToken = async (token) => {
        ...
        // .then(response => pasreTheTokenGetFromRefreshToken(response))
        // console.log("set the refreshToken again in getTokenFromRefreshToken.");
        // setRefreshToken(response.refresh);
        // console.log("set the token again in getTokenFromRefreshToken.");
        // setToken(response.access);
    ...
    useEffect(() => {
        console.log("UseEffect starts in app component for updating the previouslogin variable");
        if (cookies.previousLogin === "true") {
            console.log("we found previouslogin cookie is true. Now set the userAuthenticatedStatus variable value true.");
            setuserAuthenticatedStatus(true);
        }
        else {
            console.log("There is cookies.previouslogin in false or undefined");
        }
        console.log("UseEffect ends in app component for updating the previouslogin variable");
    }, [cookies.previousLogin])

    // // const getDataMain = async (token) => {
    // //     console.log("getdata starts.");
    // //     console.log("Here received token is : " + token);
    ...
    return (
        <BrowserRouter>
            <Routes>
                <Route index element={userAuthenticatedStatus == false ? <Login gettoken={getToken} /> : <Todo token={token} refreshToken={refreshToken} />} />
